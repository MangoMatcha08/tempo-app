...
export const testEmulatorConnection = functions.https.onCall(async (data, context) => {
  console.log('Test emulator connection called with data:', data);
  
  return {
    success: true,
    timestamp: Date.now(),
    receivedData: data,
    message: 'Firebase Emulator connection confirmed',
    isEmulator: process.env.FUNCTIONS_EMULATOR === 'true'
  };
});

// Add the sendTestNotification function for testing
export const sendTestNotification = functions.https.onCall(async (data, context) => {
  console.log('sendTestNotification called with data:', data);
  
  // Extract the notification type
  const { type = 'push', email, emulatorMode } = data;
  
  // Log if we're running in emulator mode
  if (emulatorMode) {
    console.log('Function is being called from an emulator environment');
  }
  ...
  try {
    ...
    if (type === 'email') {
      ...
      // In emulator, we don't actually send emails, just log it
      console.log(`EMULATOR: Would send email to ${email}`);
      ...
      // In emulator, we don't actually send push notifications
      console.log(`EMULATOR: Would send push notification`);
      
      return {
        success: true,
        type: 'push',
        timestamp: Date.now(),
        message: 'Test push notification would be sent'
      };
    } else {
      throw new functions.https.HttpsError(
        'invalid-argument',
...
function createPushPayload(
  ...
  switch (notificationType) {
    case NotificationTypes.UPCOMING:
      data.timeUntilDue = "30 minutes";
      break;
    case NotificationTypes.OVERDUE:
      notification.title = `Overdue: ${title}`;
      data.overdue = "true";
      break;
    case NotificationTypes.DAILY_SUMMARY:
